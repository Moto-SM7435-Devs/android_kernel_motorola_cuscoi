...
#  Motorola moto g35 (cuscoi) | SM7435 / Snapdragon 7s Gen 2 (Parrot)
#  android-16
# =============================================================================
...
#  §3  DEVICE & IDENTITY
# ═══════════════════════════════════════════════════════════════════════════════
export DEVICE="${DEVICE:-Motorola moto g35}"
...
# Exact fragment chain — matches the official build command:
#   scripts/kconfig/merge_config.sh -m -r -y \
#     arch/arm64/configs/gki_defconfig \
#     arch/arm64/configs/vendor/parrot_perf.config \
#     arch/arm64/configs/vendor/ext_config/moto-parrot.config \
#     arch/arm64/configs/vendor/ext_config/moto-parrot-cuscoi.config
CONFIG_FRAGMENTS=(
    "arch/arm64/configs/vendor/parrot_perf.config"
    "arch/arm64/configs/vendor/ext_config/moto-parrot.config"
    "arch/arm64/configs/vendor/ext_config/moto-parrot-${CODENAME}.config"
)
...
# Save the current generated .config back into the source tree
save_defconfig() {
...
dtb() {
    ...
    run_make -j"${PROCS}" dtbs 2>&1 | tee -a "${LOG_FILE}" || abort "DTB build failed"
    ...
    for target in dtbo dtbo.img; do
        if run_make -s help 2>/dev/null | awk '{print $1}' | grep -qx "${target}"; then
            msg "Building ${target}"
            run_make -j"${PROCS}" "${target}" 2>&1 | tee -a "${LOG_FILE}" ||
