#!/bin/bash
# High-Performance Kernel 6.6 Unified Build Script (Inspired by cyberknight777's kramel.sh)
# Set up to run directly from your main kernel root folder.

# --- ANSI Terminal Styling (cyberknight777 style) ---
RED="\e[1;31m"
GREEN="\e[1;32m"
YELLOW="\e[1;33m"
BLUE="\e[1;34m"
...
build_artifacts() {
    ...
    # Sweep both out/ and source sub-trees to pull all compiled driver objects (.ko)
...
case "$1" in
    ...
    *)
        ...
        if [ $BUILD_RESULT -eq 0 ]; then
            echo -e "${GREEN}***************************************************${NC}"
            echo -e "${GREEN}  SUCCESS: Kernel Compilation Process Completed!  ${NC}"
            echo -e "${GREEN}  Total Build Duration: $((DIFF / 60)) minute(s) and $((DIFF % 60)) second(s).${NC}"
            echo -e "${GREEN}  Flashable outputs ready inside: /dist              ${NC}"
            echo -e "${GREEN}***************************************************${NC}"
            exit 0
        else
            echo -e "${RED}!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!${NC}"
